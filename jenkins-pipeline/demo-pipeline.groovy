throttle(['external_grid']) {
    node ('ctm-specialproject') {
        ...
        stage ("Create ReportPortal Config Properties") {
            container("${mavenContainerVersion}"){
                if (fileExists('src/reportportal.properties')) {
                    def buildUser = ''
                    wrap([$class: 'BuildUser']) {
                        buildUser = "timer"
                    }
                    def props = readProperties  file: 'src/reportportal.properties'
                    rpEndpoint="${props['rp.endpoint']}"
                    rpUuid="${props['rp.uuid']}"
                    rpProject="${props['rp.project']}"
                    rpLaunch="${props['rp.launch']}_${environment}"
                    rpTags='"'+"${props['rp.tags']};TestRunBy=${buildUser};ScreenshotEnabled=true;Browser=${browser};Application=${application};Package=${packageName}"+'"'
                    rpDescription='"'+"${props['rp.description']}\n JenkinsBuildUrl # ${env.BUILD_URL}\n Test Run Parameters: \n -> enableGrid=true \n -> environment=${environment} \n -> testUrl=${testUrl} \n -> testDataFile=${testDataFile} \n -> testngFile=${testngFilePath} \n -> gridURL=${env.GRIDLASTIC_URL}"+'"'
                    echo "ReportPortal Launch Name # ${rpLaunch}"
                    echo "ReportPortal Launch Tags # ${rpTags}"
                }
            }
        }
                ...
                script {
                    try {
                        configFileProvider([configFile(fileId: 'mvn_settings', targetLocation: 'settings.xml')]) {
                            sh """mvn --settings settings.xml -B -q test -DpackageName="${packageName}"   -DtestName="${testName}" -DbrowserName="${browser}" -DenableGrid=true -Denvironment="${environment}" -DtestUrl="${testUrl}" -DtestDataFile="${testDataFile}" -DtestngFile=${testngFilePath} -DgridURL=${gridURL} -Dapplication="${application}" -Drp.endpoint=${rpEndpoint} -Drp.uuid=${rpUuid} -Drp.project=${rpProject} -Drp.launch=${rpLaunch} -Drp.tags=${rpTags} -Drp.description=${rpDescription} -Drp.mode=DEFAULT"""
                            //cleanWs()
                        ...
                        //cleanWs()
                        if(manager.logContains(".*Suite Execution Complete.*")) {
